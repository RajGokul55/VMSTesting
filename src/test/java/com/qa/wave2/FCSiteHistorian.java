...
public class FCSiteHistorian extends CommonMethods {
	...
	By runMode_status = By.cssSelector("[role='gridcell'][col-id='RunMode']");
	...
	public void TM_FCSiteHistorian_S03_VerifyMyView() {
		...
		List<WebElement> runModeStatus = createWebElementsBy(runMode_status);
		for (int i = 0; i < runModeStatus.size(); i++) {
			scrollToWebElement(runModeStatus.get(i));
			IsDisplayed_IsEnabled(runModeStatus.get(i));
			Assert.assertEquals(runModeStatus.get(i).getText(), "Continuous");
		}
		takeScreenshotAtEndOfTest();
